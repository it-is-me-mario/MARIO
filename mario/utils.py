...
from mario.model.conventions import INDEX_NAME_LAYOUTS, TABLE_LEVELS
    ...
    for key, value in _dict.items():
        for item in ["index", "columns"]:
            if isinstance(getattr(value, item), pd.MultiIndex):
                getattr(value, item).names = INDEX_NAME_LAYOUTS["3levels"]
            else:
                getattr(value, item).name = INDEX_NAME_LAYOUTS["1level"]
