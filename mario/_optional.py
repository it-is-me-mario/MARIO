...
def require_pyarrow(*, feature: str, error_type: type[Exception] = ImportError) -> None:
    ...
    try:
        ...
        raise error_type(
            f"{feature} requires the optional dependency 'pyarrow>=17'. "
            "Install it with `pip install 'mariopy[parquet]'` or "
            "`pip install 'pyarrow>=17'`. If `pyarrow` is already installed, "
