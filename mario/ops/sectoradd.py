...
from mario.utils import multiindex_contain
...
def parse_add_sector_excels(
    ...
    sheet = ADD_SECTOR_SHEETS[key]["sheet"]
    index_col = list(range(ADD_SECTOR_SHEETS[key]["rows"]))
    header = list(range(ADD_SECTOR_SHEETS[key]["cols"]))
...
def _infer_new_sector_names(data: Dict, empty_matrices: Dict) -> list:
    ...
    for key, frame in empty_matrices.items():
        ...
        if key in ["v", "e"]:
            candidates = list(frame.columns.get_level_values(-1))
            existing = set(data[key].columns.get_level_values(-1))
        else:
            candidates = list(frame.index.get_level_values(-1))
            existing = set(data[key].index.get_level_values(-1))

        new_sectors = []
        for item in candidates:
            if item not in existing and item not in new_sectors:
...
def matrix_concat(data: Dict, empty_matrices: Dict, new_sectors: list | None = None) -> None:
    """concat the original data to the newly create one and sort the index"""
    if new_sectors is None:
        new_sectors = _infer_new_sector_names(data, empty_matrices)
