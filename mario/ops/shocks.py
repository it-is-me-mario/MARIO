...
from mario.ops.workbook_specs import SHOCK_COLUMNS
...
def Y_shock(instance, path, boolean, clusters, to_baseline):
    ...
    if boolean:
        ...
        row_region = list(info[SHOCK_COLUMNS["r_reg"]].values)
        row_level = list(info[SHOCK_COLUMNS["r_lev"]].values)
        row_sector = list(info[SHOCK_COLUMNS["r_sec"]].values)
        column_region = list(info[SHOCK_COLUMNS["c_reg"]].values)
        demand_category = list(info[SHOCK_COLUMNS["d_cat"]].values)
        _type = list(info[SHOCK_COLUMNS["type"]].values)
        value = list(info[SHOCK_COLUMNS["value"]].values)
...
def V_shock(instance, path, matrix, boolean, clusters, to_baseline):
    ...
    if boolean:
        ...
        row_sector = list(info[SHOCK_COLUMNS["r_sec"]].values)
        column_region = list(info[SHOCK_COLUMNS["c_reg"]].values)
        column_level = list(info[SHOCK_COLUMNS["c_lev"]].values)
        column_sector = list(info[SHOCK_COLUMNS["c_sec"]].values)
        _type = list(info[SHOCK_COLUMNS["type"]].values)
        value = list(info[SHOCK_COLUMNS["value"]].values)
