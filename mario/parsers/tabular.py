...
def exio3(path, version):
    ...
    read["matrices"]["V"] = read["matrices"]["F"].loc[EXIO_FACTOR_ROWS, :]
    read["matrices"]["E"] = read["matrices"]["F"].drop(EXIO_FACTOR_ROWS)
    read["matrices"]["EY"] = read["matrices"]["EY"].drop(EXIO_FACTOR_ROWS)

    del read["matrices"]["F"]

    indeces = {}
    for key, value in EXIO_INDEX_LAYOUT.items():
